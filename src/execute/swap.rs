...
pub fn calculate_swap(
    ...
    let (input_reserve, output_reserve, trade_volume_in_erth) = if input_token == &config.erth_token_contract {
        (
            pool_info.state.token_erth_reserve,
        ...
        (
            ...
            (input_amount * pool_info.state.token_erth_reserve) / pool_info.state.token_b_reserve,
        )
    } else {
    ...
    if input_token == &config.erth_token_contract {
        pool_info.state.token_erth_reserve += amount_after_protocol_fee; // Add input amount to ERTH reserve
        ...
        pool_info.state.token_erth_reserve -= output_amount;          // Subtract from ERTH reserve (as we are sending this amount)
        ...
        pool_info.state.token_erth_reserve -= protocol_fee_in_erth;
...
pub fn anml_buyback_swap(
    ...
    // Update reserves
    pool_info.state.token_erth_reserve += amount;
...
fn calculate_feeless_swap(
    ...
    let (input_reserve, output_reserve) = if input_token == &pool_info.config.token_b_contract {
        (pool_info.state.token_b_reserve, pool_info.state.token_erth_reserve)
    } else if input_token == &config.erth_token_contract {
        (pool_info.state.token_erth_reserve, pool_info.state.token_b_reserve)
    } else {
